...
pub fn parse_amount(raw: &str) -> Option<Decimal> {
    ...
    if let Some(inner) = s.strip_prefix('(').and_then(|v| v.strip_suffix(')')) {
        return Some(-inner.trim().parse::<Decimal>().ok()?);
    }
    s.parse().ok()
}
...
pub fn import_file(
    ...
    for row in &parsed_rows {
        ...
        conn.execute(
            ...
            rusqlite::params![account_id, row.date, row.description, row.amount.to_string(), import_id],
        )?;
        imported += 1;
    }
...
mod tests {
    ...
    fn test_parse_amount() {
        ...
        assert_eq!(parse_amount("\"500.00\""), Some(dec!(500.0)));
        assert_eq!(parse_amount("  -42.50  "), Some(dec!(-42.5)));
    ...
    fn test_parse_amount_parenthesized_negatives() {
        assert_eq!(parse_amount("(500.00)"), Some(dec!(-500.0)));
        assert_eq!(parse_amount("(1,234.56)"), Some(dec!(-1234.56)));
        assert_eq!(parse_amount("\"(50.00)\""), Some(dec!(-50.0)));
    }
