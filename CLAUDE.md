...
- **TOTP:** `totp.rs` — optional TOTP two-factor authentication module (feature-gated behind `totp`); uses `totp-rs` for code generation/verification (SHA1, 6-digit, 30s window, 1-step skew) and `keyring` for cross-platform OS keychain storage (macOS Keychain, Windows Credential Store, Linux Secret Service/keyutils); TOTP secret stored in OS keychain (service=`"nigel"`, user=`"totp:<canonical_db_path>"`), `totp_enabled` flag in metadata table; `generate_secret()`, `verify_code()`, `enable()`/`disable()`, `get_secret()`, `is_enabled()`, `prompt_totp_if_needed()`; recovery codes: 8 single-use `XXXX-XXXX` codes (unambiguous charset, SHA-256 hashed) stored in metadata key `totp_recovery_codes`; `generate_recovery_codes()`, `verify_recovery_code()`, `store_recovery_codes()`, `spend_recovery_code()`, `remaining_recovery_codes()`, `clear_recovery_codes()`; escape hatches in CLI prompt, splash screen, and password manager TUI when keychain is unavailable or TOTP attempts exhausted
...
nigel password totp-recover                       # Recover access via recovery code (disables 2FA)
...
- TOTP secret is stored in the OS keychain via `keyring` crate (never in the database); the database stores only a `totp_enabled` flag and hashed recovery codes in the `metadata` table; TOTP enrollment is device-bound (tied to the machine's keychain); 8 single-use recovery codes are generated at setup; recovery escape hatches exist in CLI (`prompt_totp_if_needed`, `nigel password totp-recover`), splash screen, and password manager TUI for when the keychain is unavailable or TOTP attempts are exhausted
...
src/
  ...
  cli/                  # CLI subcommands
    ...
    password.rs         # nigel password set/change/remove/totp-enable/totp-disable/totp-recover
