...
- **Rules Manager:** `cli/rules_manager.rs` — inline TUI screen for viewing and deleting categorization rules; scrollable list with soft-delete confirmation; reads through the shared `cli::rules::list_rules` data layer
...
src/
  ...
  server/               # Web server (feature-gated behind "serve")
    ...
    routes/
      ...
      rules.rs          # GET /api/rules
      imports.rs        # GET /api/imports, GET /api/csv-profiles
