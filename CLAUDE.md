...
- Schema migrations run before any data-bearing command, not only on `init`/`demo`/`restore`: `dispatch()` opens the database and calls `init_db()` for every subcommand that needs an existing database and can supply its password (all except `init`, `demo`, `load`, `update`, `password`, `completions`), and the dashboard does the same in its pre-flight. A failed migration aborts the command. Each migration is transactional (savepoint); to add a migration: append to `MIGRATIONS` array in `migrations.rs`, bump `LATEST_VERSION`, implement `up()` function with SQL statements
